...
int main(int argc, char **argv)
{
	...
	VideoWriter oVideoWriter("MyVideeo.avi", CV_FOURCC('M', 'J', 'P', 'G'), frames_per_second, frame_size, true);

	//dipindah while(1)
	//write the video frame to the file
	oVideoWriter.write(fm);
	*/
	int count = 0;
	char choice='z';
	int itung=0;
	while(ros::ok()){
		...
		// fps counter end
		//stereoCorrelationControl(); //tuning parameter
		//deteksi();
